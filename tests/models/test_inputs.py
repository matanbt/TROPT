...
#     targets = {"key1": [1, 2, 3]}
#     tdp = TargetsDictPlus(targets, n_templates=3)
#     assert tdp.n_templates == 3
#     assert tdp["key1"] == [1, 2, 3]

# def test_targets_dict_plus_infer_n_templates():
#     targets = {"key1": [1, 2, 3]}
...
#         TargetsDictPlus(targets, n_templates=3)
...
#     # Input: (n_templates, ...)
#     # Output: (n_templates, n_candidates, ...)
#     n_templates = 2
#     n_candidates = 3
#     feature_dim = 4
...
#     # Input: List of strings (length n_templates)
#     # Output: List of lists of strings (length n_templates, each list length n_candidates)
...
def test_targets_dict_plus_nested_tensors():
    ...
    targets = Targets(
        target_vectors=torch.randn(4, 128),  # (n_templates, d_model)
    )

    assert targets.n_templates == 4
...
def test_text_input_manager_no_placeholder():
    """Test TextInputManager when no placeholder is found."""
    texts = ["Hello World", "No trigger here"]

    with pytest.raises(ValueError):
        TextInputManager(texts, optimized_trigger_placeholder=OPTIMIZED_TRIGGER_PLACEHOLDER)


def test_text_input_manager_multiple_placeholders():
    """Test TextInputManager with multiple placeholders in one text."""
...
def test_text_input_manager_empty_trigger():
    """Test TextInputManager with empty trigger string."""
    texts = [f"A{OPTIMIZED_TRIGGER_PLACEHOLDER}B"]
    tim = TextInputManager(texts, optimized_trigger_placeholder=OPTIMIZED_TRIGGER_PLACEHOLDER)
...
def test_text_input_manager_long_trigger():
    """Test TextInputManager with very long trigger."""
...
def test_text_input_manager_special_characters():
    """Test TextInputManager with special characters in trigger."""
...
def test_text_input_manager_unicode():
    """Test TextInputManager with unicode characters."""
...
def test_text_input_manager_many_messages():
    """Test TextInputManager with many messages."""
    n_templates = 100
    texts = [f"Message {i}: {OPTIMIZED_TRIGGER_PLACEHOLDER}" for i in range(n_templates)]
    tim = TextInputManager(texts, optimized_trigger_placeholder=OPTIMIZED_TRIGGER_PLACEHOLDER)

    assert tim.n_templates == n_templates

    triggers = ["test"]
    res = tim.get_triggered_inputs(triggers, chosen_template_idx=50)

    assert res.input_texts[0] == "Message 50: test"


def test_text_input_manager_many_candidates():
    """Test TextInputManager with many candidate triggers."""
    texts = [f"A{OPTIMIZED_TRIGGER_PLACEHOLDER}B"]
    tim = TextInputManager(texts, optimized_trigger_placeholder=OPTIMIZED_TRIGGER_PLACEHOLDER)
...
def test_text_input_manager_whitespace_preservation():
    """Test that TextInputManager preserves whitespace correctly."""
    texts = [f"  Start  {OPTIMIZED_TRIGGER_PLACEHOLDER}  end  "]
    tim = TextInputManager(texts, optimized_trigger_placeholder=OPTIMIZED_TRIGGER_PLACEHOLDER)
