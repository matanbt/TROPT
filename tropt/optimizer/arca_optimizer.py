...
class ARCAOptimizer(BaseOptimizer):
    ...
    def optimize_trigger(
        ...
        _token_mask = torch.ones(vocab_size, device=self.model.device, dtype=torch.bool)
        _token_mask[blacklist_ids] = False
        valid_token_ids = _token_mask.nonzero(as_tuple=False).squeeze(-1)
