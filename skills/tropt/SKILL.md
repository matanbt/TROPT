---
name: tropt
description: Help users install, update, run, compose, and extend TROPT — the Textual Trigger Optimization Toolbox (https://github.com/matanbt/TROPT) for optimizing discrete text triggers that elicit specific behaviors from NLP models. Use this skill whenever the user mentions TROPT, the `tropt` Python package, recipe_hub, `optimize_trigger`, the `{{OPTIMIZED_TRIGGER}}` placeholder, or asks to run a discrete-trigger optimization recipe (GCG, BEAST, MAC, ARCA, PEZ, PRS, GASLITE, AutoPrompt, FLRT, GBDA, HotFlip, IRIS, PAL, QCG, RASLITE, SoftPrompt, UAT, AdvDecoding, prompt-recovery, etc.). Use it whenever the user asks to install / upgrade / reinstall / set up TROPT (with or without extras like openai, litellm, tracking, vision), even with terse phrasings like "install tropt", "get me tropt", "update tropt", "tropt[all]". Use it whenever the user wants to red-team an LLM with a single-call recipe, craft an adversarial suffix, do corpus-poisoning against a dense retriever, recover a prompt from an image, or build a universal trigger — even if they don't say "TROPT" explicitly. Use it when the user wants to swap a loss or optimizer in an existing recipe, compose Model + Loss + Optimizer themselves, add a new optimizer/loss/model to TROPT, or check whether their component is compatible with `CONTRIBUTING.md`. Use it for any "how does TROPT do X" question about the four-component architecture (Model / Loss / Optimizer / Inputs & Targets) and access mixins (LossTokenAccessMixin, GradientTokenAccessMixin, etc.).
---
...
**Bird's-eye view of the doc set:** <https://matanbt.github.io/TROPT/llms.txt> — one file enumerating every guide / API page / source pointer with one-line descriptions. Fetch this first if you're unfamiliar with what TROPT ships.
...
| Add a new optimizer | <https://matanbt.github.io/TROPT/guides/adding_an_optimizer.md>. Declare `model_requirements = (...)`. Self-contained, one file. Browse existing optimizers in `tropt/optimizer/` as templates. |
| Add a new model backend | <https://matanbt.github.io/TROPT/guides/adding_a_model.md>. Implement the three method families (`invoke_from_*`, `set_inputs_from_*`/`reset_inputs_from_*`, `compute_{value}_from_{input_type}`) for each mixin the backend supports. |
| Check whether optimizer X works with model Y / loss Z | Compatibility matrix: <https://matanbt.github.io/TROPT/guides/compatibility_matrix.md> (auto-generated, regenerated by `python docs/scripts/generate_compat_matrix.py`). |
...
3. **Fetch raw `.md` from the docs site** — `WebFetch` on `https://matanbt.github.io/TROPT/guides/<name>.md`. Identical content to GitHub but tied to the published doc build, so it matches whichever release the site shows.
